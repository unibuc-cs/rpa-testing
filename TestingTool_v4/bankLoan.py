...
if (method == 'OFFLINEALL'):
	start_time = time.time()
	#call fuzzer
	os.system("python "+fuzzer_path+" -modelBasePath "+json_path+" -workflowsSpecInput "+ json_name+" -solverStrategy STRATEGY_OFFLINE_ALL "+" -debug_outputGraphFile "+graph_path+" -outputTests_PrefixFile "+results_path+" -outputTests_MaxTestPerFile 5 -debug_tests_fullPaths 0 -debug_consoleOutput 0 -debug_tests_fullVariablesContent 0")
	execution_time = time.time() - start_time
	logs_path = "../../C#Models/SimpleBankLoanCSharp/executionLogs.txt"
	# open the file in the write mode
	f= open(logs_path,"a+")
	f.write("Execution time fuzzer offline all paths %f seconds\n" % execution_time)
	f.close()
elif (method == 'DFSSYMBOLIC'):
	start_time = time.time()
	#call fuzzer
	...
	start_time = time.time()
	#call fuzzer
	os.system("python "+fuzzer_path+" -modelBasePath "+json_path+" -workflowsSpecInput "+ json_name+" -solverStrategy STRATEGY_CONCOLIC "+" -debug_outputGraphFile "+graph_path+" -outputTests_PrefixFile "+results_path+" -outputTests_MaxTestPerFile 10 -debug_tests_fullPaths 0 -debug_consoleOutput 0 -debug_tests_fullVariablesContent 1 -seedsFile inputSeeds.csv -numRandomGeneratedSeeds 0")
	execution_time = time.time() - start_time
    logs_path = "../../C#Models/SimpleBankLoanCSharp/executionLogs.txt"
    # open the file in the write mode
    f= open(logs_path,"a+")
    f.write("Execution time fuzzer concolic %f seconds\n" % execution_time)
    f.close()